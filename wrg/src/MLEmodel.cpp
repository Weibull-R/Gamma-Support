...
// if left(i) should become zero or less, then this data needs to be handled as if left censored on right(i)
			...
			if(N[0]>0)  {			
				for(int i=0; i<N[0]; i++)  {		
					failcomp=failcomp+nf(i)*R::dgamma(fail(i)-tz, par(0),  1/par(1), 1);	
				}		
			}			
...
//  Need to exempt any zero or negative values						
					if(susp(i)-tz>0)  {	
						suscomp=suscomp+ns(i)*R::pgamma(susp(i)-tz, par(0), 1/par(1), 0, 1);
					}	
				}		
			}			
			if(N[2]>0)  {			
				for(int i=0; i<N[2]; i++)  {		
					discomp=discomp+nd(i)*log(1-R::pgamma(disc(i)-tz,par(0), 1/par(1),0,0));	
				}		
			}			
			if(N[3]>0)  {			
				for(int i=0; i<N[3]; i++)  {		
// if left(i) should become zero or less, then this data needs to be handled as if left censored on right(i)						
					if(left(i)-tz>0)  {	
						intcomp=intcomp+ni(i)*log(
						R::pgamma(left(i)-tz,par(0), 1/par(1),0,0) -
						R::pgamma(right(i)-tz,par(0), 1/par(1),0,0)
						);
					}else{	
						intcomp=intcomp+ni(i)*log(1-R::pgamma(right(i)-tz,par(0), 1/par(1),0,0));
					}	
				}		
			}			
